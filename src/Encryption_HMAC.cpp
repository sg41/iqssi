...
const std::string Encryption::Timestamp =
    std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::system_clock::now().time_since_epoch())
                       .count());
const std::string Encryption::RecvWindow = "5000";

std::string Encryption::GeneratePostSignature(
...
std::string Encryption::ComputeSignature(const std::string& data) {
  unsigned char* digest = HMAC(
      EVP_sha256(), ApiSecret.c_str(), static_cast<int>(ApiSecret.length()),
...
void Encryption::PlaceOrder() {
  nlohmann::json parameters = {{"category", "linear"}, {"symbol", "BTCUSDT"},
                               {"side", "Buy"},        {"positionIdx", 0},
                               {"orderType", "Limit"}, {"qty", "0.001"},
                               {"price", "18900"},     {"timeInForce", "GTC"}};

  std::string signature = Encryption::GeneratePostSignature(parameters);
  std::string jsonPayload = parameters.dump();

  httplib::SSLClient client("api-testnet.bybit.com",
                            443);  // 443 is the default port for HTTPS
  httplib::Headers headers = {{"X-BAPI-API-KEY", ApiKey},
  ...
  auto res =
      client.Post("/v5/order/create", headers, jsonPayload, "application/json");
  if (res) {
...
void Encryption::GetOpenOrder() {
  std::map<std::string, std::string> parameters = {{"category", "linear"},
                                                   {"symbol", "BTCUSDT"}};
